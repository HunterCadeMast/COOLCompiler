...
class BasicBlock:
    def __init__(self, block_id, instructions):
        ...
        self.outgoing_edges = []  # List of successor blocks (for control flow)

    def add_edge(self, successor_block):
        self.outgoing_edges.append(successor_block)
...
def deserialize_expression(lines):
    ...
    if expression_name == "assign":
        ...
        while stack:
            ...
            if isinstance(current_expr, (Plus, Minus, Times, Divide)):
                if isinstance(current_expr.e1, Variable) and current_expr.e1.name == assignee_name:
                    if assignee_name not in mutables:
                        mutables.append(assignee_name)
                if isinstance(current_expr.e2, Variable) and current_expr.e2.name == assignee_name:
                    if assignee_name not in mutables:
                        mutables.append(assignee_name)
                if isinstance(current_expr.e1, (Plus, Minus, Times, Divide)):
                    stack.append(current_expr.e1)
                if isinstance(current_expr.e2, (Plus, Minus, Times, Divide)):
                    stack.append(current_expr.e2)
    ...
    # For dynamic dispatch and static dispatch, we need to make sure we check to object type for it possibly being new, an identifier, a string, or self dispatch.
        ...
        if lines[index + 1] == "new":
            ...
            for _ in range(num_arguments):
                argument = deserialize_expression(lines)
                arguments.append(argument)
                if isinstance(argument, Variable):
                    if argument.name not in protected_variables:
                        protected_variables.append(argument.name)
...
# All used variables from each instruction for liveness analysis.
def variables_used(exp):
    if isinstance(exp, Variable):
        return {exp.name}
    elif isinstance(exp, Assign):
        return variables_used(exp.rhs)
    elif isinstance(exp, (Plus, Minus, Times, Divide)):
        return variables_used(exp.e1) | variables_used(exp.e2)
    elif isinstance(exp, If):
        return variables_used(exp.predicate) | variables_used(exp.then_body) | variables_used(exp.else_body)
    elif isinstance(exp, While):
        return variables_used(exp.predicate) | variables_used(exp.body)
    elif isinstance(exp, Block):
        return set.union(*(variables_used(expression) for expression in exp.exps))
    elif isinstance(exp, Let):
        binds = set(var_name for var_name, exp_type, init in exp.binds)
        body_vars = variables_used(exp.body)
        return body_vars - binds
    elif isinstance(exp, (Not, Negate)):
        return variables_used(exp.exp)
    elif isinstance(exp, IsVoid):
        return variables_used(exp.void_type)
    elif isinstance(exp, Dispatch):
        return variables_used(exp.ro) | set().union(*(variables_used(arg) for arg in exp.formals))
    elif isinstance(exp, (Int, String, Bool, New)):
        return set()
    return set()

# All defined variables from each instruction for liveness analysis.
def variables_defined(exp):
    if isinstance(exp, Assign):
        return {exp.assignee}
    elif isinstance(exp, Let):
        defined_vars = set(var_name for var_name, exp_type, init in exp.binds)
        init_vars = set()
        for _, _, init in exp.binds:
            if init is not None:
                init_vars |= variables_defined(init)
        return defined_vars | init_vars
    elif isinstance(exp, Block):
        return set.union(*(variables_defined(expression) for expression in exp.exps))
    elif isinstance(exp, (If, While)):
        return set()
    return set()

# Here is where we perform livenss analysis for each type of expression.
def liveness_analysis(exp, live_out, live_map = None):
    if live_map is None:
        live_map = {}
    elif isinstance(exp, Block):
        live = live_out
        for expression in reversed(exp.exps):
            live = liveness_analysis(expression, live, live_map)
        return live
    elif isinstance(exp, Assign):
        used_vars = variables_used(exp.rhs)
        live_in = (live_out - {exp.assignee}) | used_vars
        live_map[exp] = live_in
        return live_in
    elif isinstance(exp, If):
        then_live = liveness_analysis(exp.then_body, live_out, live_map)
        else_live = liveness_analysis(exp.else_body, live_out, live_map)
        predicate_live = liveness_analysis(exp.predicate, then_live | else_live, live_map)
        live_map[exp] = predicate_live
        return predicate_live
    elif isinstance(exp, While):
        body_live = liveness_analysis(exp.body, live_out, live_map)
        predicate_live = liveness_analysis(exp.predicate, body_live, live_map)
        live_map[exp] = predicate_live | live_out
        return predicate_live | live_out
    elif isinstance(exp, Let):
        live = live_out
        for var_name, exp_type, init in reversed(exp.binds):
            if init:
                live = liveness_analysis(init, live, live_map)
            live.discard(var_name)
        live = liveness_analysis(exp.body, live, live_map)
        live_map[exp] = live
        return live
    elif isinstance(exp, Dispatch):
        used_vars = variables_used(exp.ro) | set().union(*(variables_used(arg) for arg in exp.formals))
        live_in = live_out | used_vars
        live_map[exp] = live_in
        return live_in
    else:
        live_in = live_out | variables_used(exp)
        live_map[exp] = live_in
        return live_in

# Using out live_map, we go through and remove any dead instructions.
def dead_code_elimination(exp, live_map, parent_map, class_map, class_name):
    # We start off by getting each of the successors we inherit (Attributes and method values).
    inherited_attributes = []
    current_class = class_name
    while current_class:
        if current_class in class_map:
            inherited_attributes.extend(class_map[current_class])
        current_class = parent_map.get(current_class, None)
    live_out = live_map.get(exp, set()) | {attr.id for attr in inherited_attributes}
    if isinstance(exp, Block):
        new_exps = []
        for expression in exp.exps:
            expression_check = dead_code_elimination(expression, live_map, parent_map, class_map, class_name)
            if expression_check is not None:
                new_exps.append(expression_check)
        exp.exps = new_exps
        return exp
    elif isinstance(exp, Assign):
        if exp.assignee not in live_out:
            return None
        return exp
    elif isinstance(exp, If):
        exp.predicate = dead_code_elimination(exp.predicate, live_map, parent_map, class_map, class_name)
        exp.then_body = dead_code_elimination(exp.then_body, live_map, parent_map, class_map, class_name)
        if exp.else_body:
            exp.else_body = dead_code_elimination(exp.else_body, live_map, parent_map, class_map, class_name)
        return exp
    elif isinstance(exp, While):
        exp.predicate = dead_code_elimination(exp.predicate, live_map, parent_map, class_map, class_name)
        exp.body = dead_code_elimination(exp.body, live_map, parent_map, class_map, class_name)
        return exp
    elif isinstance(exp, Let):
        for var_name, exp_type, init in exp.binds:
            init = dead_code_elimination(init, live_map, parent_map, class_map, class_name)
            if var_name not in live_out:
                return None
        exp.body = dead_code_elimination(exp.body, live_map, parent_map, class_map, class_name)
        return exp
    elif isinstance(exp, Dispatch):
        exp.formals = [dead_code_elimination(arg, live_map, parent_map, class_map, class_name) for arg in exp.formals]
        return exp
    elif isinstance(exp, Variable):
        if exp.name not in live_out:
            return None 
        return exp
    return exp

# Here, we try and reduce constants and other arithmetic operations.
def constant_propagation(exp, constants_map, class_map):
    global optimization_check
    global mutables
    if isinstance(exp, Variable):
        if exp.name in constants_map:
            old_exp = exp
            new_exp = None
            if exp.name not in mutables:
                new_exp = constants_map[exp.name]
                if old_exp != new_exp:
                    optimization_check = True
                    return new_exp
        return exp
    elif isinstance(exp, Assign):
        if isinstance(exp.assignee, Variable):
            if exp.assignee.name in mutables:
                return exp
        exp.rhs = constant_propagation(exp.rhs, constants_map, class_map)
        # We replace right hand sides with constants here.
        if isinstance(exp.rhs, Int):
            if isinstance(exp.assignee, Variable):
                constants_map[exp.assignee.name] = exp.rhs
            else:
                constants_map[exp.assignee] = exp.rhs
        else:
            if isinstance(exp.assignee, Variable):
                constants_map.pop(exp.assignee.name, None)
            else:
                constants_map.pop(exp.assignee, None)
        return exp
    elif isinstance(exp, Block):
        for i, expression in enumerate(exp.exps):
            exp.exps[i] = constant_propagation(expression, constants_map, class_map)
        return exp
    elif isinstance(exp, If):
        exp.predicate = constant_propagation(exp.predicate, constants_map, class_map)
        exp.then_body = constant_propagation(exp.then_body, constants_map, class_map)
        exp.else_body = constant_propagation(exp.else_body, constants_map, class_map)
        # Boolean values can sometimes remove branches.
        if isinstance(exp.predicate, Bool):
            if exp.predicate.value:
                return exp.then_body
            else:
                return exp.else_body
        return exp
    elif isinstance(exp, Let):
        for i, (var_name, exp_type, init) in enumerate(exp.binds):
            if init:
                exp.binds[i] = (var_name, exp_type, constant_propagation(init, constants_map, class_map))
                if isinstance(exp.binds[i][2], Int) and var_name not in mutables:
                    constants_map[var_name] = exp.binds[i][2]
        if exp.body:
            exp.body = constant_propagation(exp.body, constants_map, class_map)
        return exp
    elif isinstance(exp, While):
        exp.predicate = constant_propagation(exp.predicate, constants_map, class_map)
        exp.body = constant_propagation(exp.body, constants_map, class_map)
        return exp
    # We actually reduce constants here and treat them as variables.
    elif isinstance(exp, (Plus, Minus, Times, Divide)):
        if isinstance(exp.e1, (Plus, Minus, Times, Divide)):
            exp.e1 = constant_propagation(exp.e1, constants_map, class_map)
        if isinstance(exp.e2, (Plus, Minus, Times, Divide)):
            exp.e2 = constant_propagation(exp.e2, constants_map, class_map)
        if isinstance(exp.e1, Variable):
            if exp.e1.name in constants_map:
                exp.e1 = Int(exp.line_number, int(constants_map[exp.e1.name].value))
                optimization_check = True
        if isinstance(exp.e2, Variable):
            if exp.e2.name in constants_map:
                exp.e2 = Int(exp.line_number, int(constants_map[exp.e2.name].value))
                optimization_check = True
        if isinstance(exp.e1, Int) and isinstance(exp.e2, Int):
            if isinstance(exp, Plus):
                result = Int(exp.line_number, int(exp.e1.value) + int(exp.e2.value))
            elif isinstance(exp, Minus):
                result = Int(exp.line_number, int(exp.e1.value) - int(exp.e2.value))
            elif isinstance(exp, Times):
                result = Int(exp.line_number, int(exp.e1.value) * int(exp.e2.value))
            elif isinstance(exp, Divide):
                result = Int(exp.line_number, int(exp.e1.value) // int(exp.e2.value) if int(exp.e2.value) != 0 else 0)
            optimization_check = True
            return result
        # Below are some of the arithmetic reductions we perform such as 'x * 1' reducing to just 'x'.
        elif isinstance(exp.e1, Int):
            if isinstance(exp, Plus):
                if exp.e1.value == 0:
                    optimization_check = True
                    return exp.e2
                ...
                if exp.e1.value == 1:
                    ...
                    optimization_check = True
                    return Int(exp.line_number, 0)
        elif isinstance(exp.e2, Int):
            if isinstance(exp, Plus):
                if exp.e2.value == 0:
                    optimization_check = True
                    return exp.e1
        ...
        exp.exp = constant_propagation(exp.exp, constants_map, class_map)
        if isinstance(exp.exp, Int):
            result = Int(exp.line_number, -int(exp.exp.value))
            optimization_check = True
            return result
    elif isinstance(exp, IsVoid):
        exp.void_type = constant_propagation(exp.void_type, constants_map, class_map)
        if isinstance(exp.void_type, Int):
            result = Bool(exp.line_number, int(exp.void_type.value) == 0)
            optimization_check = True
            return result
    # Booleans can sometimes be reduced here if they are both integers.
    elif isinstance(exp, Eq):
        exp.e1 = constant_propagation(exp.e1, constants_map, class_map)
        exp.e2 = constant_propagation(exp.e2, constants_map, class_map)
        if isinstance(exp.e1, Int) and isinstance(exp.e2, Int):
            optimization_check = True
            return Bool(exp.line_number, int(exp.e1.value) == int(exp.e2.value))
        ...
        if isinstance(exp.e1, Int) and isinstance(exp.e2, Int):
            optimization_check = True
            return Bool(exp.line_number, int(exp.e1.value) <= int(exp.e2.value))
    # Unboxing if we have types from Int, Bool, or String.
    elif isinstance(exp, New):
        if isinstance(exp.new_type, Int):
            optimization_check = True
            return Int(exp.line_number, exp.new_type.value)
        elif isinstance(exp.new_type, Bool):
            optimization_check = True
            return Bool(exp.line_number, exp.new_type.value)
        elif isinstance(exp.new_type, String):
            optimization_check = True
            return String(exp.line_number, exp.new_type.value)
    return exp

# Here, we go through and get every instruction that can be replaced and removed.
def assignment_propagation(exp, assignments):
    global optimization_check
    global protected_variables
    if isinstance(exp, Assign):
        if isinstance(exp.assignee, Variable) and exp.assignee.name in protected_variables:
            return exp
        if isinstance(exp.rhs, Variable):
            if exp.rhs.name in assignments:
                optimization_check = True
                return Assign(exp.line_number, exp.assignee, assignments[exp.rhs.name])
        if isinstance(exp.assignee, Variable):
            assignments[exp.assignee.name] = exp.rhs
        else:
            assignments[exp.assignee] = exp.rhs
        exp.rhs = assignment_propagation(exp.rhs, assignments)
        if isinstance(exp.assignee, Variable):
            if exp.assignee.name not in protected_variables and exp.assignee.name not in assignments:
                optimization_check = True
                return None
        if exp.assignee not in protected_variables:
            optimization_check = True
            return None
        return exp
    elif isinstance(exp, Block):
        new_exps = []
        for expression in exp.exps:
            optimized_exp = assignment_propagation(expression, assignments)
            if optimized_exp:
                new_exps.append(optimized_exp)
        exp.exps = new_exps
        return exp
    elif isinstance(exp, If):
        exp.predicate = assignment_propagation(exp.predicate, assignments)
        exp.then_body = assignment_propagation(exp.then_body, assignments)
        exp.else_body = assignment_propagation(exp.else_body, assignments)
        return exp
    elif isinstance(exp, While):
        exp.predicate = assignment_propagation(exp.predicate, assignments)
        exp.body = assignment_propagation(exp.body, assignments)
        return exp
    elif isinstance(exp, Let):
        for i, (var_name, exp_type, init) in enumerate(exp.binds):
            if init:
                exp.binds[i] = (var_name, exp_type, assignment_propagation(init, assignments))
                if isinstance(exp.binds[i][2], Variable) and exp.binds[i][2].name in assignments:
                    exp.binds[i] = (var_name, exp_type, assignments[exp.binds[i][2].name])
                    optimization_check = True
        if exp.body:
            exp.body = assignment_propagation(exp.body, assignments)
        return exp
        ...
        for formal in exp.formals:
            formal = assignment_propagation(formal, assignments)
        return exp
    elif isinstance(exp, Negate):
        exp.exp = assignment_propagation(exp.exp, assignments)
        return exp
    elif isinstance(exp, (Eq, Lt, Le)):
        exp.e1 = assignment_propagation(exp.e1, assignments)
        exp.e2 = assignment_propagation(exp.e2, assignments)
        return exp
    elif isinstance(exp, IsVoid):
        exp.void_type = assignment_propagation(exp.void_type, assignments)
        return exp
...
# We run our optimizer on the AST here.
def optimize_ast(ast, parent_map, class_map):
    global optimization_check
    live_map = {}
    constants_map = {}
    assignments = {}
    # We run through constant propagation multiple times with attributes and not until we have reduced all we can.
    while optimization_check:
        optimization_check = False
        for class_name, attributes in class_map.items():
            processed_attributes = {}
            for attribute in attributes:
                if attribute.id not in processed_attributes:
                    attribute.init = constant_propagation(attribute.init, constants_map, class_map)
                    if isinstance(attribute.init, Int):
                        processed_attributes[attribute.id] = True
        for node in ast:
            node.exp = constant_propagation(node.exp, constants_map, class_map)
            # Uncomment this string below to run assignment propagation.
            """node.exp = assignment_propagation(node.exp, assignments)"""
    # Uncomment this string to run dead code elimination.
    """dead_code_check = True
    previous_live_map = {}
    while dead_code_check:
        dead_code_check = False
        for node in ast:
            if not isinstance(node.exp, Internal):
                liveness_analysis(node.exp, set(), live_map)
        for node in ast:
            if not isinstance(node.exp, Internal):
                node.exp = dead_code_elimination(node.exp, live_map, parent_map, class_map, node.class_name)
        if live_map != previous_live_map:
            dead_code_check = True
            previous_live_map = live_map"""
    return ast, class_map

ast, class_map = optimize_ast(copy.deepcopy(ast), copy.deepcopy(parent_map), copy.deepcopy(class_map))
...
# Here, we go through and peephole optimization.
def peephole_optimization(instructions):
    used_registers = set()
    free_registers = {f"r{i}" for i in range(8)}
    free_registers.add("fp")
    free_registers.add("ra")
    optimized_instructions = []
    previous_instruction = None
    for current_instruction in instructions:
        # If there are any instructions that push and pop with nothing between it, then we remove them.
        if isinstance(current_instruction, PushInstruction):
            if isinstance(previous_instruction, PopInstruction) and previous_instruction.recv == current_instruction.recv:
                previous_instruction = None
                continue
            used_registers.add(current_instruction.recv)
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            optimized_instructions.append(current_instruction)
        elif isinstance(current_instruction, PopInstruction):
            if isinstance(previous_instruction, PushInstruction) and previous_instruction.recv == current_instruction.recv:
                previous_instruction = None
                continue
            if current_instruction.recv in used_registers:
                used_registers.remove(current_instruction.recv)
            free_registers.add(current_instruction.recv)
            optimized_instructions.append(current_instruction)
        # Move will also remove any instruction that are redundant.
        elif isinstance(current_instruction, MoveInstruction):
            if current_instruction.recv == current_instruction.source:
                continue
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            used_registers.add(current_instruction.recv)
            if current_instruction.source in free_registers:
                free_registers.remove(current_instruction.source)
            used_registers.add(current_instruction.source)
            optimized_instructions.append(current_instruction)
        elif isinstance(current_instruction, (AddInstruction, SubtractInstruction, TimesInstruction, DivideInstruction)):
            if current_instruction.recv not in used_registers:
                continue
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            used_registers.add(current_instruction.recv)
            if current_instruction.op1 in free_registers:
                free_registers.remove(current_instruction.op1)
            used_registers.add(current_instruction.op1)
            if current_instruction.op2 in free_registers:
                free_registers.remove(current_instruction.op2)
            used_registers.add(current_instruction.op2)
            optimized_instructions.append(current_instruction)
        # Just like pushes and pops, we remove any storing and loading that happen right next to each other as sometimes, we don't need to store then.
        elif isinstance(current_instruction, StoreInstruction):
            if isinstance(previous_instruction, LoadDoublewordInstruction) and previous_instruction.address == current_instruction.address and previous_instruction.recv == current_instruction.value:
                previous_instruction = None
                continue
            if current_instruction.address in free_registers:
                free_registers.remove(current_instruction.address)
            used_registers.add(current_instruction.address)
            if current_instruction.value in free_registers:
                free_registers.remove(current_instruction.value)
            used_registers.add(current_instruction.value)
            optimized_instructions.append(current_instruction)
        elif isinstance(current_instruction, LoadDoublewordInstruction):
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            used_registers.add(current_instruction.recv)
            if current_instruction.address in free_registers:
            ...
            used_registers.add(current_instruction.address)
            optimized_instructions.append(current_instruction)
        elif isinstance(current_instruction, LoadAddressInstruction):
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            used_registers.add(current_instruction.recv)
            if current_instruction.label in free_registers:
                free_registers.remove(current_instruction.label)
            used_registers.add(current_instruction.label)
            optimized_instructions.append(current_instruction)
        elif isinstance(current_instruction, AllocateInstruction):
            if current_instruction.recv in free_registers:
                free_registers.remove(current_instruction.recv)
            used_registers.add(current_instruction.recv)
            optimized_instructions.append(current_instruction)
...
def main():
    ...
    for cls in classes:
        ...
        for i, attribute in enumerate(class_map[cls]):
            ...
            if attribute.type == "unboxed_int":
                ...
                implementation_map["self"] = implementation_map[cls]
                symbol_table["SELF_TYPE"] = acc_reg.off(2)
                implementation_map["SELF_TYPE"] = implementation_map[cls]
                cgen(attribute.init, symbol_table)
    ...
    # Peephole optimization is ran here on all of the instructions.
    vtable_instructions = peephole_optimization(vtable_instructions)
    constructor_instructions = peephole_optimization(constructor_instructions)
    method_instructions = peephole_optimization(method_instructions)
    string_instructions = peephole_optimization(string_instructions)
